...
rester = Rester()
VALID_FILTERS = ["material", "property", "application",
...
def bind(app):
    ...
    def show_results(*args, **kwargs):
        if list(args)[0] is not None:
            if list(args)[-1] == "search":
                text = str(args[1])
                filters = {f: [s.strip() for s in args[i + 2].split(',')] for i, f in enumerate(
                    VALID_FILTERS) if ((list(args)[i + 2] is not None) and (args[i + 2].split(',') != ['']))}
                results = rester.search_text_with_ents(
                    text, filters, cutoff=max_results)
    ...
    @app.callback(
        [Output("search-input", "style"), Output("material-search-panel",
                                                 "style"), Output("filter-boxes", "style")],
