...
class TestSearchLogic(MatScholarWebBaseTest):
    ...
    def test_sum_all_fields_and_buttons_n_submits(self):
        ...
        self.assertEqual(0, sum_all_fields_and_buttons_n_submits(*all_n_clicks))
        ...
        self.assertEqual(0, sum_all_fields_and_buttons_n_submits(*all_n_clicks))
        all_n_clicks[1] = 1
        self.assertEqual(1, sum_all_fields_and_buttons_n_submits(*all_n_clicks))
        all_n_clicks = [1] * n_fields
        self.assertEqual(n_fields, sum_all_fields_and_buttons_n_submits(*all_n_clicks))

    def test_search_bar_live_display(self):
        test_txt_src = 'abcdefghi'
        ent_txts = [test_txt_src[i] for i, _ in enumerate(valid_search_filters)]
        n_clicks = 0
        display = search_bar_live_display(n_clicks, *ent_txts)
        truth = [f"{valid_search_filters[i]}: {test_txt_src[i]}" for i, _ in enumerate(valid_search_filters)]
        truth = ", ".join(truth) + ","
